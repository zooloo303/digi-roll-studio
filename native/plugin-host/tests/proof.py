...
with tempfile.TemporaryDirectory(prefix='drs-host-proof-') as tmp:
    root = Path(tmp)
    def run(name, rate=48000, block=256, level=None, state=None, bad=None):
        out = root / name
        out.mkdir()
        spec = {'path': plugin}
        ...
        cfg = {'rate': rate, 'block': block, 'seconds': .1, 'output': str(out),
               'plugins': [spec, spec], 'events': events}
    ...
    cfg={'rate':48000,'block':256,'seconds':.1,'output':str(out),
         'plugins':[{'path':plugin}],
...
print('PASS: six rate/block cases; exact stereo impulses across block boundaries and two instances while stopped; zero parameter; fresh-process state recall; stopped/play/seek/tempo process context; invalid input/missing plugin')
